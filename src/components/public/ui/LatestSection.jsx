    ...
    <section className="w-full mb-20">
      ...
      <div className="relative flex items-center justify-center w-full group">
        ...
        >
          {items?.[index]?.Image ? (
            <img
              src={items[index].Image?.startsWith('http') ? items[index].Image : items[index].Image ? `/${items[index].Image}` : '/default-image.jpg'}
              alt={items[index].Title ?? 'Item'}
          ...
          <div className="p-5 flex flex-col justify-between">
            ...
            <div className="flex justify-between text-xs md:text-sm text-gray-500">
              ...
              <span>{items[index].DateCreated ? new Date(items[index].DateCreated).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) : ''}</span>
            </div>
          </div>
        </button>
