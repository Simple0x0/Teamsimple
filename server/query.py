...
SELECT
  m.month AS date,
  COALESCE(COUNT(l.LikeID), 0) AS total_likes
FROM months m
LEFT JOIN LikeLogs l ON DATE_FORMAT(l.DateCreated, '%%Y-%%m') = m.month
GROUP BY m.month
ORDER BY m.month;
"""

LIKES_BY_LAST_30_DAYS = """
WITH days AS (
...
SELECT
  ...
  COALESCE(COUNT(l.LikeID), 0) AS total_likes
FROM days d
LEFT JOIN LikeLogs l ON DATE(l.DateCreated) = d.day
GROUP BY d.day
ORDER BY d.day;
"""

LIKES_BY_LAST_7_DAYS = """
WITH days AS (
  SELECT CURDATE() - INTERVAL seq DAY AS day
  FROM (
    SELECT 0 seq UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3
...
SELECT
  ...
  COALESCE(COUNT(v.VisitorID), 0) AS total_visitors
FROM months m
LEFT JOIN Visitor v ON DATE_FORMAT(v.LastVisit, '%%Y-%%m') = m.month
GROUP BY m.month
ORDER BY m.month;
"""

VISITORS_BY_LAST_7_DAYS = """
WITH days AS (
  SELECT CURDATE() - INTERVAL seq DAY AS day
  FROM (
    SELECT 0 seq UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3
